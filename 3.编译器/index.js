...
function parse(str) {
  ...
  while(tokens.length) {
    ...
    const praent = elementStack[elementStack.length - 1]
    ...
    switch(element.type) {
      case "tag":
        ...
        praent.children.push(elementNode)
...
function traverseNode(ast, context) {
  ...
  for (let i = 0; i < nodeTransforms.length; i++) {
    const exitFunc = nodeTransforms[i](ast, context)
    if (exitFunc) {
      exitFuncs.push(exitFunc)
    }
...
function transformA(ast) {
  return () => {
    console.log(ast, "exit");
  }
}

const a = parse("<div>abc</div>")
